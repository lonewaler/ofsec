/**
 * OfSec V3 — Threat Intelligence Page Module
 */
import { api } from '../api.js';
import { toast, now } from '../ui.js';
...
export async function checkIP() {
    ...
    try {
        const data = await api('/api/v1/recon/passive', { method: 'POST', body: { target: ip, modules: ['osint'] } });
        ...
        result.innerHTML = renderRiskBar(riskPct, riskColor)
            ...
            + renderShodanCard(shodan) + renderVTCard(vt) + '</div>';
...
export async function lookupDomain() {
    ...
    try {
        const data = await api('/api/v1/recon/passive', { method: 'POST', body: { target: domain, modules: ['dns', 'whois', 'subdomain', 'osint'] } });
        ...
        html += `<div class="card" style="margin-bottom:12px;display:flex;gap:16px;align-items:center"><div style="font-size:28px">🌐</div><div style="flex:1"><div style="font-size:16px;font-weight:700">${domain}</div><div style="font-size:12px;color:var(--text-muted)">`;
        ...
        html += renderDNSCard(dns) + renderVTCardDomain(vt) + '</div>';

        const allSubs = [...new Set([...(Array.isArray(subdomains) ? subdomains : []), ...(shodan?.subdomains || [])])];
        if (allSubs.length > 0) {
            html += '<div class="card" style="margin-bottom:12px"><div style="font-size:11px;font-weight:600;margin-bottom:8px"><span class="source-badge source-censys">🔗 SUBDOMAINS</span>';
            html += `<span style="color:var(--text-muted);font-weight:400;margin-left:6px">(${allSubs.length} total)</span></div><div style="display:flex;flex-wrap:wrap;gap:6px">`;
            allSubs.slice(0, 24).forEach(s => { html += `<span class="subdomain-chip">${s}</span>`; });
            html += '</div></div>';
        }
...
// ─── IOC Tracker ────────────────────────────────
export async function trackIOC() {
    ...
    try {
        const data = await api('/api/v1/recon/passive', { method: 'POST', body: { target: iocValue, modules: ['osint'] } });
        const risk = data?.aggregate_risk_score || 0;
        const riskLabel = risk > 0.7 ? 'HIGH RISK' : risk > 0.3 ? 'MEDIUM RISK' : 'LOW RISK';
        const riskColor = risk > 0.7 ? 'var(--accent-red)' : risk > 0.3 ? 'var(--accent-orange)' : 'var(--accent-green)';

        state.iocHistory.unshift({ ioc: iocValue, type: iocType, risk: riskLabel, riskColor, timestamp: now(), data });
        const kpiIocs = document.getElementById('kpi-iocs');
        if (kpiIocs) kpiIocs.textContent = state.iocHistory.length;

        const tableBody = document.getElementById('ioc-table-body');
        if (tableBody) {
            tableBody.innerHTML = state.iocHistory.slice(0, 20).map(entry => `
        <tr>
          <td style="font-family:monospace;font-size:12px">${entry.ioc}</td>
          <td><span class="source-badge source-censys">${entry.type.toUpperCase()}</span></td>
          <td><span style="color:${entry.riskColor};font-weight:600;font-size:12px">${entry.risk}</span></td>
          <td style="color:var(--text-muted);font-size:11px">${entry.timestamp}</td>
        </tr>
      `).join('');
        }
...
export async function runIntelSweep() {
    ...
    try {
        await api('/api/v1/defense/intel/sweep', { method: 'POST' });
        toast('IOC sweep started', 'success');
        setTimeout(async () => {
...
function renderRiskBar(pct, color) {
    return `<div class="card" style="margin-bottom:12px"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px"><span style="font-weight:600">Aggregate Risk Score</span><span style="color:${color};font-size:20px;font-weight:700">${pct}%</span></div><div style="background:rgba(255,255,255,0.1);border-radius:4px;height:8px"><div class="risk-bar-fill" style="background:${color};width:${pct}%;height:100%;border-radius:4px"></div></div></div>`;
}
...
function renderShodanCard(shodan) {
    ...
    if (shodan?.error) { html += `<span class="line-warning">${shodan.error}</span>`; }
    else {
...
function renderVTCard(vt) {
    ...
    if (vt?.error) { html += `<span class="line-warning">${vt.error}</span>`; }
    else {
        const repColor = (vt?.reputation || 0) < 0 ? 'var(--accent-red)' : 'var(--accent-green)';
        html += `<div style="font-size:12px;line-height:2"><div><span style="color:var(--text-muted)">Reputation:</span> <span style="color:${repColor}">${vt?.reputation ?? '—'}</span></div>`;
        html += `<div style="display:flex;gap:8px;margin-top:4px"><span style="background:rgba(239,68,68,0.2);color:var(--accent-red);padding:3px 8px;border-radius:4px;font-size:12px">🔴 ${vt?.detections?.malicious ?? 0} Malicious</span>`;
        html += `<span style="background:rgba(251,191,36,0.2);color:var(--accent-orange);padding:3px 8px;border-radius:4px;font-size:12px">🟡 ${vt?.detections?.suspicious ?? 0} Suspicious</span></div>`;
...
function renderDNSCard(dns) {
    let html = '<div class="card"><div style="font-size:11px;font-weight:600;margin-bottom:8px"><span class="source-badge source-censys">🔍 DNS RECORDS</span></div><div style="font-size:12px;line-height:1.8">';
...
function renderVTCardDomain(vt) {
    ...
    if (vt?.error) { html += `<span class="line-warning">${vt.error}</span>`; }
        ...
        html += `<div style="font-size:12px;line-height:1.8"><div style="display:flex;gap:8px;margin-bottom:6px"><span style="background:rgba(239,68,68,0.2);color:var(--accent-red);padding:2px 8px;border-radius:4px">🔴 ${vt?.detections?.malicious ?? 0} Malicious</span>`;
        html += `<span style="background:rgba(251,191,36,0.2);color:var(--accent-orange);padding:2px 8px;border-radius:4px">🟡 ${vt?.detections?.suspicious ?? 0} Suspicious</span></div>`;
        if (vt?.registrar) html += `<div><span style="color:var(--text-muted)">Registrar:</span> ${vt.registrar}</div>`;
        if (vt?.categories && Object.keys(vt.categories).length) html += `<div><span style="color:var(--text-muted)">Category:</span> ${Object.values(vt.categories).slice(0, 2).join(', ')}</div>`;
        html += '</div>';
    }
    return html + '</div>';
}

export function initThreatsPage() {
    document.getElementById('btn-check-ip')?.addEventListener('click', checkIP);
    document.getElementById('btn-lookup-domain')?.addEventListener('click', lookupDomain);
    document.getElementById('btn-track-ioc')?.addEventListener('click', trackIOC);
    // Enter key submits IOC tracker
    document.getElementById('ioc-track-input')?.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') trackIOC();
    });
}
