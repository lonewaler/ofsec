/**
 * OfSec V3 — Defense Operations Page Module
 */
import { api } from '../api.js';
import { now } from '../ui.js';
import { onPageEnter, onPageExit } from '../router.js';
...
async function loadDefenseAlerts() {
    try {
        ...
        try { alertsRes = await api('/api/v1/defense/alerts?limit=50'); } catch { alertsRes = { alerts: [] }; }
        try { corrRes = await api('/api/v1/defense/correlation/alerts?limit=20'); } catch { corrRes = { alerts: [] }; }
...
function renderAlertsTable(alerts, hasNew) {
    ...
    body.innerHTML = alerts.map((a, i) => {
        ...
        return `<tr class="${isNew ? 'alert-new' : ''}" style="transition:background 0.5s">
...
export function initDefensePage() {
    onPageEnter('defense', startAlertPolling);
    onPageExit('defense', stopAlertPolling);
    window.addEventListener('beforeunload', stopAlertPolling);
    document.getElementById('btn-refresh-alerts')?.addEventListener('click', loadDefenseAlerts);
}
